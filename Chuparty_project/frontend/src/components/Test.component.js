...
export default class Test extends Component {
    ...
    showTest() {
        let res = (<div style={{ height: "50%" }}>
            <QuestionsCarousel parentAnswersSubmitHandler={this.answersSubmitHandler} questions={this.state.exam.questions} />
        </div>)
        return res;
    }
    showResults() {
        let res = (
            <TestResult items={this.state.submitedItems}></TestResult>
        )
        return res;
    }
    ...
    render() {
        ...
        if (this.state.loading)
            res = <div><h1>LOADING</h1></div>
        else if (this.state.afterTest)
            res = <div>{this.showResults()}</div>
        else
            res = <div>{this.showTest()}</div>
        return (res)
    }
